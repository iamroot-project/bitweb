...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-prunedebuglogfile", _("Prune (limit) filesize of debug.log"));
    if (showDebug)
    {
...
std::string LicenseInfo()
{
    ...
    const std::string Copyright_3 = strprintf(_("Copyright (C) %i-%i"), 2021, 2022) + " " + "The Bitweb Core developers";
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if bitweb is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bitweb is started while in a temporary directory.\n",
